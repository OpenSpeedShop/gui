...
IConnectionPage *ServerConnection::page()
{
    return new ServerConnectionPage(this, NULL);
}
...
void ServerConnection::readReady()
{
...
void ServerConnection::error(QAbstractSocket::SocketError error)
{
...
QString ServerConnection::errorMessage()
{
...
void ServerConnection::readSettings()
{
    ...
    bool okay;
    m_Port = settingManager.value("ServerConnection/Port").toInt(&okay);
    if(!okay) m_Port = 2048;

    settingManager.endGroup();
}

IConnection::States ServerConnection::state()
{
    return m_State;
}

void ServerConnection::setState(States state)
{
    m_State = state;
    emit stateChanged();
}

void ServerConnection::send(QString command)
{
    if(!command.endsWith('\n'))
        command.append('\n');
...
QString ServerConnection::receive()
{
    if((quint32)m_Buffer.count() >= m_BufferSize) {
